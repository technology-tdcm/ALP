...
async function migrateAndSeed() {
    console.log('Running migration and seed...\n');

    // Create tables via SQL RPC (using Supabase's SQL endpoint)
    ...
    if (testError && testError.code === '42P01') {
        console.log('Tables not found. Please run supabase-migration.sql in Supabase SQL Editor first.\n');
        console.log('Go to: https://hahqatijgujbvnorlqaf.supabase.co/project/default/sql');
        console.log('Copy and paste the contents of supabase-migration.sql and run it.\n');
        console.log('After that, run: npm run seed\n');
        return;
    }
    ...
    if (!existingUnis || existingUnis.length === 0) {
        ...
        console.log('✓ Universities seeded (8)');
    }
    ...
    if (!existingPC || existingPC.length === 0) {
        ...
        for (let i = 0; i < pcData.length; i += 50) {
            const batch = pcData.slice(i, i + 50);
            await supabase.from('page_content').insert(batch);
        }
        console.log('✓ Page content seeded (' + pcData.length + ' entries)');
    }
